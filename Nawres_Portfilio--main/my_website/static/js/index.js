...
document.addEventListener("DOMContentLoaded", function (event) {
  ...
  function typeText() {
    const text = "Machine Learning & Computer Vision Engineer | Full-Stack Developer";
    const element = document.getElementById("animate-text-1");
    let index = 0;

    // Clear the element's content to prevent duplication
    element.textContent = "";
    ...
    function type() {
      if (index < text.length) {
        ...
        index++;
        setTimeout(type, 100); // Adjust 100ms to change typing speed
      }
    }
